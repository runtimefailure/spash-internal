...
namespace http
{
    ...
    int httpget(lua_State* L)
    {
        ...
        return YieldExecution(L, [Url, Headers]() -> std::function<int(lua_State*)>
            {
                cpr::Response Result;
                try
                {
    ...
    int request(lua_State* L)
    {
        ...
        {
            //Roblox::Print(1, "URL called: %s", Url.c_str());
            Url = "https://postman-echo.com/get?foo=bar"; // sometimes https://httpbin.org/user-agent doesnt work properly so it just change website so it wont crash
        }
